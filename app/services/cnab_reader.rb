...
class CnabReader
  ...
  def execute
    File.readlines(@file_path).each { |x| create_store_operation(x) }
  end
  ...
  def create_store_operation(content)
    StoreOperation.create(build_store_operation(content))
  end

  def get_attribute(content, *attribute_names)
    attribute_names.map do |attribute|
      content[attributes_range[attribute][:start]...attributes_range[attribute][:end]]
    end.join(' ')
  end
