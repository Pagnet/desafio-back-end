...
As transações que foram salvas no banco devem ser expostas em um endpoint com um retorno em JSON. Este endpoint deve ser seguro o bastante para não sofrer exposição pública das informações sigilosas referente as transações salvas na base dados.

O arquivo precisa ser parseado da seguinte maneira:

# Documentação do CNAE
